...
Source: https://sarahconstantin.substack.com/p/why-i-am-not-an-ai-doomer

---

[![](https://substackcdn.com/image/fetch/$s_!JmCy!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F2a753167-c24b-4d3c-9379-5a83f3fda59f_1024x1024.png)](https://substackcdn.com/image/fetch/$s_!JmCy!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F2a753167-c24b-4d3c-9379-5a83f3fda59f_1024x1024.png)
...
We changed the [climate and geology](https://en.wikipedia.org/wiki/Anthropocene) of our planet. If you look at Earth from space, human agency is obvious \-\- you can see land transformed by agriculture, continents illuminated by electric lights, the Great Wall of China, satellites in orbit.
...
This is [apparently controversial](https://betterwithout.ai/about-you), which is why I’m belaboring the point. Some people do not actually think humans are “agents” who solve problems in pursuit of goals, and I think this is silly. I think humans do pursue goals.
...
An adversary is far more dangerous than an accident. Note, by analogy, that [only 1% of gun deaths in the US](https://www.pewresearch.org/fact-tank/2022/02/03/what-the-data-says-about-gun-deaths-in-the-u-s/) are accidental. If you die from a gunshot wound, it is two orders of magnitude more likely to be a death from human agency (homicide or suicide) than a death by accident.
...
There are numerous examples of AIs engaging in what DeepMind calls “[specification gaming”](https://www.deepmind.com/blog/specification-gaming-the-flip-side-of-ai-ingenuity) \-\- “discovering” that the easiest way to minimize their loss function is to do something very far from what their human creators intended.
...
Nostalgebraist’s posts  “[why assume AGIs will optimize for fixed goals?](https://www.lesswrong.com/posts/dKTh9Td3KaJ8QW6gw/why-assume-agis-will-optimize-for-fixed-goals)” and “[wrapper\-minds are the enem](https://www.lesswrong.com/posts/Mrz2srZWc7EzbADSo/wrapper-minds-are-the-enemy)y” points at this same distinction.
...
> “A literallyunconditional love would not be a love for a person, for any entity, but only for the referent of an imagined XML tag, defined only inside one's own mind.”
> 
> …
> 
...
Yohei Nakajima, a venture capitalist, has scared a lot of people by launching what he calls “[task\-driven autonomous agents](https://yoheinakajima.com/task-driven-autonomous-agent-utilizing-gpt-4-pinecone-and-langchain-for-diverse-applications/)”, basically a couple of GPT\-4 instances hooked up to a memory store.
...
For instance, Yann LeCun describes a “world model” as “[a kind of simulator of the part of the world relevant to the task at hand](https://ai.facebook.com/blog/yann-lecun-advances-in-ai-research/)”, which is of course just replacing the word “model” with the synonym “simulator”.
...
In mathematical language, you would say that the relationships between the model’s parts *[commute with](https://en.wikipedia.org/wiki/Commutative_diagram)* the relationship between the model and the world.
...
This sort of world model, [it turned out](https://people.csail.mit.edu/brooks/papers/BrooksIJCAI91.pdf), was neither necessary nor optimal for successfully solving robotics problems. A robot could simply sense patterns and react to them appropriately, without generating a model of the entire space around itself and a pre\-planned path through it.

Herbert Simon’s [parable of the ant](https://rs.io/herbert-simons-ant/) is an attempt to explain that world models are not inherently necessary for effective coping in varied environments. He imagines an ant that chooses its zigzag path along the beach without any kind of a priori “plan” and without any “map” of the beach.  The ant simply moves in a straight line until it encounters an obstacle, and then zigs around it. The ant’s path appears complex as seen from above, but it emerges from a set of simple rules. And it’s perfectly capable of successfully avoiding obstacles on a variety of different beaches.
...
Cognitive scientist Joshua Tenenbaum has argued that humans learn to reason about physical objects \-\- to predict when objects stacked or leaning against each other will balance or fall over, for instance \-\- using an “[intuitive physics model](https://www.pnas.org/doi/abs/10.1073/pnas.1306572110)” that approximates Newtonian mechanics.

Children learn to walk and manipulate objects relatively quickly, and they generalize to all kinds of environments, which suggests that they need compact “models” that predict how objects behave in general.  Preverbal infants are capable of [predicting](https://citeseerx.ist.psu.edu/document?repid=rep1&type=pdf&doi=433fe3ec915dae4d56b8779f95ddd4e9cdcba8a6) the outcomes of complex physical scenarios they have never seen, in ways that are consistent with “intuitive physics models.”

And learning to predict events [happens much faster and needs less data](https://dspace.mit.edu/bitstream/handle/1721.1/70135/Tenenbaum-2011-Learning%20a%20theory%20of%20causality.pdf?sequence=1&isAllowed=y) for models that learn a causal theory compared with models that learn no theory.  So it’s plausible that humans *do*, and AIs *should*, learn causal theories to explain the world, in order to learn faster from less data.
...
When this occurs, you get the famous “adversarial examples”, such as when a [deep learning image classification model makes dramatic misclassification errors](https://arxiv.org/pdf/2009.03728.pdf) if small, invisible\-to\-humans amounts of apparent high\-frequency noise are added to the image.
...
Noam Chomsky’s 1959 [attack](https://chomsky.info/1967____/) on B.F. Skinner’s behaviorist theory of language argued that humans do not learn language through reinforcement learning. Almost all children learn to speak, whether they are carefully rewarded by adults for speaking correctly or not. And we can all comprehend novel sentences that we have never seen before, and even notice grammatical errors in them, which we would not be able to do if Skinner were correct and each hyper\-specific “behavior” had to be trained separately with no general principles relating them.

With language, as with navigating physical environments, we can only explain humans’ ability to learn quickly and generalize by positing what Tenenbaum calls “[intuitive theories](http://cbmm.mit.edu/sites/default/files/publications/Intuitive%20Theories%20(Gerstenberg,%20Tenenbaum,%202016.pdf)”: inexplicit, compact representations of generalized “laws of nature” or causal beliefs about how things work. We develop an “intuitive physics” to explain how objects behave in space, an “intuitive grammar” to explain how language works, an “intuitive psychology” to explain people’s behavior,
...
Many of the tasks on the [BIG\-Bench](https://github.com/google/BIG-bench/blob/main/bigbench/benchmark_tasks/README.md) benchmark set involve verbal reasoning questions that attempt to probe “structural” or “world\-model” understanding, such as recognizing [cause and effect](https://github.com/google/BIG-bench/tree/main/bigbench/benchmark_tasks/cause_and_effect) in text, [deriving the rules of an invented language](https://github.com/google/BIG-bench/tree/main/bigbench/benchmark_tasks/conlang_translation) from a few examples, [predicting counterfactual scenarios](https://github.com/google/BIG-bench/tree/main/bigbench/benchmark_tasks/crass_ai), [solve multistep arithmetic problems](https://github.com/google/BIG-bench/tree/main/bigbench/benchmark_tasks/multistep_arithmetic), and so on.

All l[arge language models up to GPT\-3 are much worse than humans](https://arxiv.org/pdf/2206.04615.pdf) at BIG\-Bench tasks, but they improve predictably with scale.

([We don’t know](https://cdn.openai.com/papers/gpt-4.pdf) how GPT\-4 performs on BIG\-Bench because they used the test questions in their training set; whoops!)
...
This is a [familiar historical pattern](https://srconstantin.wordpress.com/2017/01/28/performance-trends-in-ai/); image classification was once utterly out of reach for machine learning models, until it wasn’t and started improving proportionally to compute with the first CNNs in 2012; the same went for machine translation, where performance was stalled at “basically nonexistent” until the models got big enough.
...
[![](https://substackcdn.com/image/fetch/$s_!wr3p!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F8d31a234-b4d1-4ee2-82c4-6d97e2f556b5_1515x567.png)](https://substackcdn.com/image/fetch/$s_!wr3p!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F8d31a234-b4d1-4ee2-82c4-6d97e2f556b5_1515x567.png)
...
Finally, AlphaZero also is sensitive to [adversarial perturbations](https://openreview.net/attachment?id=yZ_JlZaOCzv&name=supplementary_material) in Go \-\- adding two irrelevant stones to a board state can make the AI make wrong moves that human players can trivially avoid. In fact, using these adversarial moves, a human amateur was [recently able to beat a top AI system at Go](https://garymarcus.substack.com/p/david-beats-go-liath), 14 games to 1\.
...
In Judea Pearl’s [framework](http://bayes.cs.ucla.edu/BOOK-2K/), causality is the essential thing that causal models offer that statistical prediction models (including all current machine learning and AI models) do not.
...
There’s an active subcommunity that keeps saying we need to build AI on causal models \-\- and it includes some luminaries like [Yoshua Bengio](https://www.wired.com/story/ai-pioneer-algorithms-understand-why/), who has proposed a research direction for [causal AI](https://arxiv.org/pdf/2102.11107.pdf) that involves inferring causal relationships from data.

But this community is small and new.

It was only as recently as 2021 that researchers at Columbia University and the University of Montreal defined a framework for “[neural causal models](https://par.nsf.gov/servlets/purl/10317650)” that are trainable using gradient descent. That means that essentially *no* empirical work could get done until two years ago.
...
[Adversarial perturbations](https://dspace.mit.edu/bitstream/handle/1721.1/127541/1193031569-MIT.pdf?sequence=1&isAllowed=y) to deep learning classification of images can cause drastic misclassifications from tiny, human\-imperceptible “noise” added to images.

You don’t even necessarily need to algorithmically design adversarial images; ordinary camera noise or viewing angle variation can get a classifier trained on a different dataset [to mistake a pen for a revolver half the time](https://link.springer.com/article/10.1007/s13042-021-01435-0), simply because the type of camera used changed from the training to the test dataset.
...
With traditional (pre\-foundation models) deep learning, *adding a single new category* to a classifier requires training a [whole new network](https://www.amazon.science/blog/updating-neural-networks-to-recognize-new-categories-with-minimal-retraining) (albeit with the ability to transfer weights from the original network).
...
1. I formulate an (abstract) plan; I have an idea of what steps I’ll take, what kinds of results I might get, and what things might go wrong.
2. I get stuck on a task that I *didn’t even realize was the kind of thing that could go wrong*.
3. I now need a new ontology (tasks, results, possible roadblocks) that incorporates the new information.

[Leaky abstractions](https://www.joelonsoftware.com/2002/11/11/the-law-of-leaky-abstractions/) are everywhere in engineering.  In trying to root\-cause a software bug, you may discover the real problem is a miswired circuit board.    
...
[Reality has a surprising amount of detail.](http://johnsalvatier.org/blog/2017/reality-has-a-surprising-amount-of-detail)  Anything you have never done before \-\- building stairs, planting a vegetable garden, writing a computer program \-\- seems much simpler in the abstract and has tons of finicky little details up close.
...
This is the “[Ontological Crises](https://intelligence.org/files/OntologicalCrises.pdf)” problem. The toy example in the linked paper imagines an AI that makes moves on a simulated “world” made up of a grid of squares. If the AI is assigned to go to the ‘farthest square on the right’, and the ontology changes to increase the number of squares in the “world”, then the AI needs to find a way to revise its goal so it will *still go to the farthest\-right square in this larger world*.
...
Moreover, cross\-ontology goal robustness is required for an agent to “[view itself as embedded in a world](https://www.alignmentforum.org/posts/i3BTagvt3HbPMx6PN/embedded-agency-full-text-version).”
...
An “embedded agent” knows that what happens to “itself” can affect its chances of succeeding at its objective. It can “protect itself”, “improve itself”, “acquire resources for itself”, etc \-\- all subgoals that are [instrumentally useful](https://selfawaresystems.files.wordpress.com/2008/01/ai_drives_final.pdf) for basically any AI’s goal, and all [very plausibly x\-risks](https://gwern.net/fiction/clippy) if an AI tries to do something like “maximize available compute”.  But first it has to have some concept of “itself” as a program running on a computer in order to learn to make causal predictions about how to do such things.
